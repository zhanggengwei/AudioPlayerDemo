...
{
  if (!_decodingContextInitialized) {
    return WDAudioDecoderFailed;
  }

  pthread_mutex_lock(&_decodingContext.mutex);

  WDAudioFileProvider *provider = [_playbackItem fileProvider];
  ...
  if (ioOutputDataPackets == 0) {
    [_lpcm setEnd:YES];
    pthread_mutex_unlock(&_decodingContext.mutex);
    return WDAudioDecoderEndEncountered;
  }
  ...
  pthread_mutex_unlock(&_decodingContext.mutex);
  return WDAudioDecoderSucceeded;
}
